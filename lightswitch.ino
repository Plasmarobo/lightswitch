...
#define MDNS_RETRY    2000      # Number of milliseconds between rebroadcasting a name query
...
#define MDNS_TLD      ".lan"  # Top-level domain to search for. This can technically be anything, but pretty much everyone says leave it as .local
...
const char* password = "lightswitch";
...
//#define USE_DHCP_IP     false
  ...
  #ifdef ESP32
    IPAddress stationIP   = IPAddress(192, 168, 10, 232);
  #else
    IPAddress stationIP   = IPAddress(192, 168, 10, 186);
  #endif
  
  IPAddress gatewayIP   = IPAddress(192, 168, 10, 1);
  IPAddress netMask     = IPAddress(255, 255, 255, 0);
#endif
...
const unsigned char OUTPUT_PIN = PIN_D5;
const unsigned char INPUT_PIN = PIN_D4;

uint8_t power_state = LOW;
uint8_t last_sw_state;

void update_power(char payload)
{
  if (payload == 1)
  {
    power_state = HIGH;
  ...
  digitalWrite(OUTPUT_PIN, power_state);
}

const char* mqtt_server = "messages.lan";
const unsigned int MSG_BUFFER_SIZE = 512;
char msg[MSG_BUFFER_SIZE] = { '\0' };

const unsigned int ID_BUFFER_SIZE = 256;
char mqtt_clientid[ID_BUFFER_SIZE] = { '\0' };

const unsigned int MQTT_TOPIC_MAX_LEN = 512;
char mqtt_status_topic[MQTT_TOPIC_MAX_LEN];
char mqtt_set_topic[MQTT_TOPIC_MAX_LEN];

WiFiClient espClient;
PubSubClient mqttClient(espClient);

const char MQTT_HOSTNAME[] = "messages.lan";
const char MQTT_USERNAME[] = "lightswitch";
...
void publish_state()
{
  mqttClient.publish(mqtt_status_topic, power_state == HIGH ? "ON" : "OFF", true);
}

void message_callback(char* topic, byte* payload, unsigned int len)
{
  char term_buffer[len+1];
  memset(term_buffer, '\0', len+1);
  uint8_t newstate = power_state;
  memcpy(term_buffer, payload, len);
  unsigned int term_len = strlen(term_buffer);
  if (len == 0 || term_len == 0)
  {
    return;
  }
  if (term_len > 1 && !isdigit(term_buffer[0])) {
    if (strncmp(term_buffer, "ON", std::min(term_len, (unsigned int)2)) == 0)
    {
        newstate = HIGH;
    }

    if (strncmp(term_buffer, "OFF", std::min(term_len, (unsigned int)3)) == 0)
    {
      newstate = LOW;
    }
  } else {
    char* endptr;
    newstate = static_cast<uint8_t>(strtol(term_buffer, &endptr, 10));
  }
  if (newstate != power_state)
  {
    update_power(newstate);
    publish_state();
  }
}

void handle_sw_change()
{
  delay(50);
  uint8_t newstate = digitalRead(INPUT_PIN);
  if (newstate != last_sw_state)
  {
    last_sw_state = newstate;
    // Toggle current state
    update_power(power_state == HIGH ? LOW : HIGH);
    publish_state();
  }
}
...
void reconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(mqtt_clientid, MQTT_USERNAME, MQTT_PASSWORD)) {
      ...
      mqttClient.publish("discovery", mqtt_clientid);
      // ... and resubscribe
...
void setup()
{
  ...
  Serial.println(ESP_getChipId());
  
  memset(mqtt_clientid, ID_BUFFER_SIZE, '\0');
  snprintf(mqtt_clientid, ID_BUFFER_SIZE, "LightSwitch%08x", ESP_getChipId());
  snprintf(mqtt_status_topic, MQTT_TOPIC_MAX_LEN, "%s/status", mqtt_clientid);
  snprintf(mqtt_set_topic, MQTT_TOPIC_MAX_LEN, "%s/set", mqtt_clientid);
  ...
  last_sw_state = digitalRead(INPUT_PIN);
  ...
  Serial.println("Looking up MQTT server...");
  int8_t error_count = 0;
  Serial.print("Connecting MQTT...");
  while(udp.parsePacket() > 0);
  int res = WiFi.hostByName(MQTT_HOSTNAME, mqttIP);
  if (res != 1) {
    mDNSResolver::Resolver resolver(udp);
    resolver.setLocalIP(WiFi.localIP());
    mqttIP = resolver.search(MQTT_HOSTNAME);
    if(mqttIP == INADDR_NONE) {
      //HARDCODED IP
      Serial.println("Failing back to hardcoded IP");
      mqttIP.fromString(MQTT_SERVER_IP);
    } 
  }
  Serial.print("Got ");
...
void loop()
{
  ...
  ArduinoOTA.handle();
  yield();
  if (digitalRead(INPUT_PIN) != last_sw_state)
  {
    handle_sw_change();
  }
}
